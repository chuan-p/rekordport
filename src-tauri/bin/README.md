...
- macOS 上如果这里放的是 Homebrew 的 `sqlcipher` / `ffmpeg` / `ffprobe` 符号链接，也可以参与构建；预处理脚本会复制真实二进制并把非系统动态库一起打包。
- Apple Silicon 这组三个 sidecar 目前建议用 [tools/build-macos13-arm64-sidecars.sh](/Users/chuanpeng/Documents/rkb-lossless-process/tools/build-macos13-arm64-sidecars.sh) 生成，默认把最低系统版本压到 macOS 13.0，并保留 `aac_at` / `libmp3lame`。
