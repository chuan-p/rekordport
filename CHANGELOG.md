...
- Windows file-path clicks now open the containing folder instead of passing the audio file path to Explorer, avoiding accidental file launches and false reveal errors from Explorer exit codes.
- Split folder-opening commands so scan result paths open their containing folder, backup links open the folder itself, and errors distinguish missing paths, non-folder paths, missing containing folders, and file manager launch failures.
